...
var QS = [
  ...
  {q:'What year was JavaScript created?',      opts:['1995','2000','1985'],                                                                        ans:0},
  {q:'Which HTML tag makes the largest heading?', opts:['&lt;h6&gt;','&lt;h1&gt;','&lt;head&gt;'],                                                 ans:1},
  {q:'What does CSS stand for?',               opts:['Computer Style Sheets','Creative Style System','Cascading Style Sheets'],                    ans:2},
  {q:'Which Python library is used for data analysis?', opts:['NumPy','Pandas','Matplotlib'],                                                      ans:1},
  {q:'What does AI stand for?',                opts:['Automated Intelligence','Artificial Intelligence','Advanced Interface'],                      ans:1},
  {q:'Which tag is used for a hyperlink in HTML?', opts:['&lt;link&gt;','&lt;a&gt;','&lt;href&gt;'],                                               ans:1},
  {q:'What does ML stand for in technology?',  opts:['Meta Learning','Machine Language','Machine Learning'],                                       ans:2},
  {q:'Which CSS property changes text color?', opts:['font-color','text-color','color'],                                                           ans:2}
];
 
var qCur = 0;
var qAns = new Array(QS.length).fill(null);
var qStats = {att:0, tot:0, best:0};
try { var _s = JSON.parse(localStorage.getItem('suhaib_q') || '{}'); if(_s.att !== undefined) qStats = _s; } catch(e){}
 
function renderQ(){
  var q = QS[qCur];
  var html = '<p class="q-text">'+(qCur+1)+'. '+q.q+'</p><div class="opts">';
  q.opts.forEach(function(o, i){
    var sel = qAns[qCur] === i ? ' selected' : '';
    html += '<div class="opt'+sel+'" data-i="'+i+'"><div class="opt-dot">'+(qAns[qCur]===i?'✓':'')+'</div><span>'+o+'</span></div>';
  });
  html += '</div>';
  ...
  document.querySelectorAll('#qArea .opt').forEach(function(el){
    el.addEventListener('click', function(){
      ...
      qAns[qCur] = i;
      document.querySelectorAll('#qArea .opt').forEach(function(o){
        o.classList.remove('selected');
        o.querySelector('.opt-dot').textContent = '';
      });
      this.classList.add('selected');
      this.querySelector('.opt-dot').textContent = '✓';
    });
  });
 
  var total = QS.length;
  document.getElementById('qCtr').textContent = 'Question '+(qCur+1)+' of '+total;
  document.getElementById('qProg').style.width = (((qCur+1)/total)*100) + '%';
  document.getElementById('prevBtn').style.display = qCur > 0 ? 'inline-flex' : 'none';
  document.getElementById('nextBtn').style.display = qCur < total-1 ? 'inline-flex' : 'none';
  document.getElementById('subBtn').style.display  = qCur === total-1 ? 'inline-flex' : 'none';
}
 
document.getElementById('nextBtn').addEventListener('click', function(){
  if (qAns[qCur] === null) { toast('⚠️ Please select an answer!'); return; }
  qCur++; renderQ();
});
...
document.getElementById('subBtn').addEventListener('click', function(){
  if (qAns[qCur] === null) { toast('⚠️ Please select an answer!'); return; }
  var score = qAns.filter(function(a,i){ return a === QS[i].ans; }).length;
  var pct = Math.round(score / QS.length * 100);
  qStats.att++;
  qStats.tot += pct;
  qStats.best = Math.max(qStats.best, pct);
  try { localStorage.setItem('suhaib_q', JSON.stringify(qStats)); } catch(e){}
  updateQStats();
  // Show result
  ...
  document.getElementById('scoreRing').style.background =
    'conic-gradient(var(--accent) '+deg+'deg, var(--surface2) '+deg+'deg)';
  document.getElementById('scoreVal').textContent = pct + '%';
  document.getElementById('rMsg').textContent = score + ' / ' + QS.length + ' correct';
  document.getElementById('rSub').textContent =
    pct >= 80 ? ' Excellent! Web dev pro!' :
...
document.getElementById('restartBtn').addEventListener('click', function(){
  qCur = 0;
  qAns = new Array(QS.length).fill(null);
...
function updateQStats(){
  document.getElementById('qsAtt').textContent = qStats.att;
  document.getElementById('qsAvg').textContent = qStats.att > 0 ? Math.round(qStats.tot/qStats.att)+'%' : '0%';
  document.getElementById('qsBest').textContent = qStats.best + '%';
}
